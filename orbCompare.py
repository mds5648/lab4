...
def percent_match(finger1, finger2, distance):
    ...
    kp_a, desc_a = orb.detectAndCompute(finger1, None)
    kp_b, desc_b = orb.detectAndCompute(finger2, None)
    
    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
    
    matches = bf.match(desc_a, desc_b)
    
    similar = [match for match in matches if match.distance <= distance]
    if len(matches) == 0:
        return 0
